...
public interface ProducedGoodRepository extends JpaRepository<ProducedGood,Long>{
	...
	@Query("SELECT new com.cfs.inventory.dto.ProductReportDto(p.name,p.quantity,sum(sl.quantity)) from Product p "
	...
	@Query("SELECT new com.cfs.inventory.dto.ProductReportDto(p.name,p.quantity,sum(sl.quantity)) from Product p "
			+ "join SalesLineItem sl on p.id=sl.product.id join Sale s  on sl.sale.id = s.id "
			+ "where WEEK(s.orderDate) = :week group by p.name,p.quantity")
